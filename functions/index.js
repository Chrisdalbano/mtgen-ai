...
exports.generateCardText = functions.https.onRequest((request, response) => {
  cors(request, response, async () => {
    ...
    try {
      const gptOutput = await generateText(prompt);
      response.json({ gpt_output: gptOutput });
    } catch (error) {
      console.error("Error in generating card text:", error);
...
exports.generateCardArt = functions.https.onRequest((request, response) => {
  cors(request, response, async () => {
    if (request.method !== "POST") {
      return response.status(405).send("Method Not Allowed");
    }
    ...
    try {
      ...
      response.json({ image_urls: imageUrls });
    } catch (error) {
      console.error("Error in generating card art:", error);
      response.status(500).send({
        message: "Internal server error",
        details: error.message,
        stack: error.stack,
